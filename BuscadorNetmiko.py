...
def connect_and_execute(hostname, username, password, manufacturer, choice):
    try:
        # Definir os parâmetros do dispositivo remoto
        device = {
            'device_type': 'juniper' if manufacturer.lower() == 'juniper' else 'cisco_ios',
            'host': hostname,
            'username': username,
            'password': password,
            'secret': password,  # Se necessário, use 'secret' para habilitar o modo privilegiado
        }

        # Cria a conexão com o dispositivo remoto
